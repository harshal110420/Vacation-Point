...
const HotelsMiddle = () => {
  ...
  const specificCategory = "Luxury Hotels";
  const categoryHotels = HotelsData.filter(
    (Hotels) => Hotels.category === specificCategory
  );
  const [currentHotelsIndex, setCurrentHotelsIndex] = useState(0);
  const visibleHotels = categoryHotels.slice(
    currentHotelsIndex,
    currentHotelsIndex + 4
  );
  ...
  const isBeginning = currentHotelsIndex === 0;
  const isEnd = currentHotelsIndex >= categoryHotels.length - 4;

  const handleSlideLeft = () => {
    setCurrentHotelsIndex((prevIndex) =>
      prevIndex > 0 ? prevIndex - 1 : categoryHotels.length - 1
    );
  };

  const handleSlideRight = () => {
    setCurrentHotelsIndex((prevIndex) =>
      prevIndex < categoryHotels.length - 4 ? prevIndex + 1 : 0
    );
  };
  return (
