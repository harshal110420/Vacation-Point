...
const DestinationsMiddle = () => {
  const [loading, setLoading] = useState(true);
  const specificCategory = "Most Visited";
  const categoryDestinations = destinationsData.filter(
    (Destinations) => Destinations.category === specificCategory
  );
  const [currentDestinationsIndex, setCurrentDestinationsIndex] = useState(0);
  const visibleDestinations = categoryDestinations.slice(
    currentDestinationsIndex,
    currentDestinationsIndex + 4
  );
  ...
  const isBeginning = currentDestinationsIndex === 0;
  const isEnd = currentDestinationsIndex >= categoryDestinations.length - 4;

  const handleSlideLeft = () => {
    setCurrentDestinationsIndex((prevIndex) =>
      prevIndex > 0 ? prevIndex - 1 : categoryDestinations.length - 1
    );
  };

  const handleSlideRight = () => {
    setCurrentDestinationsIndex((prevIndex) =>
      prevIndex < categoryDestinations.length - 4 ? prevIndex + 1 : 0
    );
  };
  return (
...
export default DestinationsMiddle;
